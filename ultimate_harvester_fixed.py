...
def main():
    ...
    if username:
        ...
        found = harvester.search_social(username)
        print(f"  ✅ Found on {len(found)} platforms: {', '.join(found[:5])}{'...' if len(found) > 5 else ''}")
    
    # 2. Enter websites to scrape
    print("\n[2] Enter websites to scrape (comma separated):")
    urls_input = input("URLs: ").strip()
    if urls_input:
        urls = [u.strip() for u in urls_input.split(',') if u.strip()]
        print(f"  Scraping {len(urls)} websites...")
        with ThreadPoolExecutor(max_workers=5) as executor:
            list(executor.map(harvester.scrape_website, urls))
